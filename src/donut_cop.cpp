...
void DonutCop::handleControlMessage(const ofxOscMessage &m) {
  ...
  // Calculate left and right IDs
    ...
    for(int i = 0; i< data.size(); i++){
        str+= ofToString((int)data.getData()[i]);
        str+= ", ";
    ...
    ofLog() << str;
    if(idIdx == -1) {
        ofLog() << "Didn't find my own ID";
        return;
    }
    if(idIdx == 0){
        leftId = data.getData()[data.size()-1];
    } else{
        leftId = data.getData()[idIdx - 1];
    } rightId = data.getData()[(idIdx + 1) % data.size()];
  /*
  int maxId = 0;
  leftId = 256;
  rightId = -1;
